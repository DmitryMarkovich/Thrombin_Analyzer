...
DatasetR6$set(
    ...
    overwrite = FALSE);  ## End of DatasetR6$copy_and_analyze_TG
################################################################################
...
DatasetR6$set(
        ...
        f = function(updateProgress = NULL, progress) {
            if (!is.null(data) && length(data) != 0) {
                ...
                for (i in 2:N) {
                    ...
                    ## print(res[[signals[i]]]$parms);
                    ## print(paste0(">> res from i = ", i));
                    ## print(res);
                    ## print(tmp.res[[i - 1]]$get_parms());
                    if (!any(tmp.res[[i - 1]]$auto_model() == c("None", "T0Gamma"))) {
                        ## print("parms YEs");
                        ## print(tmp.res[[i - 1]]$get_parameter("Lagtime"));
                        ## print(tmp.res[[i - 1]]$get_parameter("ETP"));
                        ## print(tmp.res[[i - 1]]$get_parameter("Peak"));
                        ## print(tmp.res[[i - 1]]$get_parameter("ttPeak"));
                        ## print(tmp.res[[i - 1]]$get_parameter("VelIndex"));
                        ## print(tmp.res[[i - 1]]$get_parameter("Alpha2M_Level"));
    ...
    overwrite = FALSE);  ## End of DatasetR6$do_analysis
################################################################################
