...
Cal$set(
    ...
    overwrite = FALSE);  ## End of Cal$fit_LateMM
################################################################################

################################################################################
Cal$set(
    which = "public", name = "get_LateMM",
    value = compiler::cmpfun(
        f = function() {
            if (exists(x = "LateMM", where = fit)) {
                ...
                return(b + p1 * (1 - (LambertW::W(p2 * exp(p2) * exp(-p3 * data$x)) / p2)));
            } else {
                warning(">> fit$LateMM does not exist!");
                return(rep(0, length(data$x)));
            }
        }, options = kCmpFunOptions),
    overwrite = FALSE);  ## End of Cal$get_LateMM
################################################################################

################################################################################
Cal$set(
    which = "public", name = "parms_LateMM",
    value = compiler::cmpfun(
        f = function(e0, s0) {
    ...
    overwrite = FALSE);  ## End of Cal$parms_LateMM
################################################################################

################################################################################
Cal$set(
    which = "public", name = "get_init_rate_LateMM",
    value = compiler::cmpfun(
        f = function() {
            if (exists(x = "LateMM", where = fit)) {
                p1 <- fit$LateMM$cff[["p1"]]; p2 <- fit$LateMM$cff[["p2"]];
    ...
    overwrite = FALSE);  ## End of Cal$_get_init_rate_LateMM
################################################################################
